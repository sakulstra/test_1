...
export class GovDataService {
  ...
  async getVotingData(
    ...
    userAddress?: string,
    representative?: Record<number, string>,
  ) {
          ...
          if (userAddress) {
            return (
              (await votingMachineDataHelper.getProposalsData(
                ...
                representative[chainId] || userAddress || constants.AddressZero,
              )) || []
            );
          }
        }
  ...
  // representations
  async getRepresentationData(address: string, chainId: number) {
  ...
  async voteBySignature({
    ...
    if (this.signer) {
      ...
      const gelatoApiKey = appConfig.gelatoApiKeys.testnet; // TODO: need to add mainnet also
